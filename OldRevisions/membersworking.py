...
class Member(User):
    ...
    def fetch_borrowed_books(self):
        ...
        try:
            ...
            cursor.execute("SELECT books.ISBN, books.title, books.author, requests.due_date FROM requests JOIN books ON requests.item_id = books.item_id WHERE requests.user_id = ? AND requests.request_type = 'Borrow'", (self.user_id,))
    ...
    def view_profile(self): # Show user details and borrowed books/reservations
        ...
        print(f"Reserved Books: {self.__reserved_books}")
